...
public class AdapterMembers extends RecyclerView.Adapter<AdapterMembers.HolderParticipantsAdd>{
    ...
    public void onBindViewHolder(@NonNull HolderParticipantsAdd holder, int position) {
        ...
        try {
            Picasso.get().load(dp).placeholder(R.drawable.placeholder).into(holder.circularImageView);

        }catch (Exception e){
            Picasso.get().load(R.drawable.placeholder).into(holder.circularImageView);
        }
        checkifAlreadyExists(modelUser, holder,mUsername);
        checkifAlreadyExists2(modelUser, holder,mUsername);
...
//                    st.show();
    ...
    private void checkifAlreadyExists2(ModelUser modelUser, HolderParticipantsAdd holder, String mUsername) {
        DatabaseReference ref = FirebaseDatabase.getInstance().getReference("Groups");
...
//                            holder.username.setText(mUsername + " - " +hisRole);
                            if (hisRole.equals("admin")){
                                Intent intent = new Intent("groupadmin");
                                intent.putExtra("item", mUsername);
